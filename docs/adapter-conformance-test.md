...
### 6.1 MVP 内存适配器豁免规则（ACT-13）
1. 当适配器实现类型为“内存模拟（无子进程路径）”时，`ACT-13` 可标记为 `N/A`。
2. 标记 `N/A` 的前提：
- 测试报告必须明确写出“无子进程执行路径”事实与影响范围。
- 必须由 `PM-AGENT` 与 `QA-AGENT` 双签确认豁免。
- 其余 `ACT-01..12`、`ACT-14..17` 仍需全部通过。
3. 豁免有效期仅限当前 MVP 里程碑；进入真实 CLI 子进程集成后，`ACT-13` 必须恢复为必测并通过。
